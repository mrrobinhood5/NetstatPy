...
def main():
    ...
    ip_address = input("Enter in the IP address for the quest:").strip()
    ...
    # subprocess.Popen("adb tcpip 5037", shell=True)
    # sleep(3)

    # connect wireless tell it to disconnect headset
    subprocess.Popen(f'adb connect {ip_address}:5037', shell=True)
    sleep(5)
    ...
    while True:
        ...
        connected_ips = subprocess.Popen(f"adb -s {ip_address}:5555 shell netstat -tn | grep ESTABLISHED",
        ...
        for ip in connected_ips:
            ...
            p = subprocess.Popen(f"adb -s {ip_address}:5555 shell ping -c 10 {ip}", shell=True,
        ...
        for x in z:
            db_data.append({'tester_name': tester_name, 'location_info': location_info,
